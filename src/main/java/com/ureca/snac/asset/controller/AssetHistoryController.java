...
public class AssetHistoryController implements AssetHistorySwagger {
    ...
    @Override
    public ResponseEntity<ApiResponse<AssetHistoryListResponse>> getMyAssetHistories(
        ...
        AssetHistoryListResponse response = assetHistoryService.getAssetHistories(userDetails.getUsername(), request);

        log.info("[자산 내역 조회 완료] 회원 : {}, 조회된 내역 개수 : {}", userDetails.getUsername(), response.histories().size());
