...
public interface MoneyRechargeSwagger {
    ...
    @ErrorCode400(description = "충전 금액은 0보다 커야한다")
    @ErrorCode401
    ...
    @ErrorCode400(description = "결제 검증 실패")
    @ErrorCode404
    @GetMapping("/api/money/recharge/success")
    ResponseEntity<ApiResponse<Void>> rechargeSuccess(
            ...
            @Parameter(description = "토스페이먼츠 결제 고유 키", required = true) @RequestParam Long amount
    );
